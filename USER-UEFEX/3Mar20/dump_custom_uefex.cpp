...
/*
  This code is based on dump_custom.cpp
...
DumpCustomUefex::DumpCustomUefex(LAMMPS *lmp, int narg, char **arg) :
  Dump(lmp, narg, arg),
  ...
  vbuf(NULL), id_custom(NULL), flag_custom(NULL), typenames(NULL), pack_choice(NULL)
{
  ...
  if (ioptional < nfield &&
    ...
    error->all(FLERR,"Invalid attribute in dump custom/uefex command");
...
DumpCustomUefex::~DumpCustomUefex()
{
...
void DumpCustomUefex::init_style()
{
  ...
  if (binary && domain->triclinic == 0)
    header_choice = &DumpCustomUefex::header_binary;
  else if (binary && domain->triclinic == 1)
    header_choice = &DumpCustomUefex::header_binary_triclinic;
  else if (!binary && domain->triclinic == 0)
    header_choice = &DumpCustomUefex::header_item;
  else if (!binary && domain->triclinic == 1)
    header_choice = &DumpCustomUefex::header_item_triclinic;

  if (binary) write_choice = &DumpCustomUefex::write_binary;
  else if (buffer_flag == 1) write_choice = &DumpCustomUefex::write_string;
  else write_choice = &DumpCustomUefex::write_lines;
  ...
  if (i==modify->nfix)
    error->all(FLERR,"Can't use dump custom/uefex without defining a fix nve/uefex");
  ifix_uefex=i;
}

/* ---------------------------------------------------------------------- */

void DumpCustomUefex::write_header(bigint ndump)
{
  // simulation box bounds (copy from dump.cpp)
...
void DumpCustomUefex::header_binary(bigint ndump)
{
...
void DumpCustomUefex::header_binary_triclinic(bigint ndump)
{
  fwrite(&update->ntimestep,sizeof(bigint),1,fp);
  fwrite(&ndump,sizeof(bigint),1,fp);
  fwrite(&domain->triclinic,sizeof(int),1,fp);
...
void DumpCustomUefex::header_item(bigint ndump)
{
...
void DumpCustomUefex::header_item_triclinic(bigint ndump)
{
  fprintf(fp,"ITEM: TIMESTEP\n");
  fprintf(fp,BIGINT_FORMAT "\n",update->ntimestep);
  fprintf(fp,"ITEM: NUMBER OF ATOMS\n");
...
void DumpCustomUefex::pack(tagint *ids)
{
...
int DumpCustomUefex::convert_string(int n, double *mybuf)
{
  int i,j;
...
void DumpCustomUefex::write_data(int n, double *mybuf)
{
...
void DumpCustomUefex::write_binary(int n, double *mybuf)
{
  n *= size_one;
...
void DumpCustomUefex::write_string(int n, double *mybuf)
{
...
void DumpCustomUefex::write_lines(int n, double *mybuf)
{
  int i,j;
...
int DumpCustomUefex::parse_fields(int narg, char **arg)
{
  // customize by adding to if statement
  ...
  for (int iarg = 0; iarg < narg; iarg++) {
    ...
    if (strcmp(arg[iarg],"id") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_id;
      if (sizeof(tagint) == sizeof(smallint)) vtype[i] = INT;
      else vtype[i] = BIGINT;
    } else if (strcmp(arg[iarg],"mol") == 0) {
      if (!atom->molecule_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_molecule;
      if (sizeof(tagint) == sizeof(smallint)) vtype[i] = INT;
      else vtype[i] = BIGINT;
    } else if (strcmp(arg[iarg],"proc") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_proc;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"procp1") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_procp1;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"type") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_type;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"element") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_type;
      vtype[i] = STRING;
    } else if (strcmp(arg[iarg],"mass") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_mass;
      vtype[i] = DOUBLE;

    } else if (strcmp(arg[iarg],"x") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_x;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"y") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_y;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"z") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_z;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"xs") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpCustomUefex::pack_xs_triclinic;
      else pack_choice[i] = &DumpCustomUefex::pack_xs;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ys") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpCustomUefex::pack_ys_triclinic;
      else pack_choice[i] = &DumpCustomUefex::pack_ys;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"zs") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpCustomUefex::pack_zs_triclinic;
      else pack_choice[i] = &DumpCustomUefex::pack_zs;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"xu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpCustomUefex::pack_xu_triclinic;
      else pack_choice[i] = &DumpCustomUefex::pack_xu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"yu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpCustomUefex::pack_yu_triclinic;
      else pack_choice[i] = &DumpCustomUefex::pack_yu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"zu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpCustomUefex::pack_zu_triclinic;
      else pack_choice[i] = &DumpCustomUefex::pack_zu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"xsu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpCustomUefex::pack_xsu_triclinic;
      else pack_choice[i] = &DumpCustomUefex::pack_xsu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ysu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpCustomUefex::pack_ysu_triclinic;
      else pack_choice[i] = &DumpCustomUefex::pack_ysu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"zsu") == 0) {
      if (domain->triclinic) pack_choice[i] = &DumpCustomUefex::pack_zsu_triclinic;
      else pack_choice[i] = &DumpCustomUefex::pack_zsu;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"ix") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_ix;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"iy") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_iy;
      vtype[i] = INT;
    } else if (strcmp(arg[iarg],"iz") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_iz;
      vtype[i] = INT;

    } else if (strcmp(arg[iarg],"vx") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_vx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"vy") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_vy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"vz") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_vz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fx") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_fx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fy") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_fy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"fz") == 0) {
      pack_choice[i] = &DumpCustomUefex::pack_fz;
      vtype[i] = DOUBLE;

    } else if (strcmp(arg[iarg],"q") == 0) {
      if (!atom->q_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_q;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"mux") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_mux;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"muy") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_muy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"muz") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_muz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"mu") == 0) {
      if (!atom->mu_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_mu;
      vtype[i] = DOUBLE;

    } else if (strcmp(arg[iarg],"radius") == 0) {
      if (!atom->radius_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_radius;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"diameter") == 0) {
      if (!atom->radius_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_diameter;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"omegax") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_omegax;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"omegay") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_omegay;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"omegaz") == 0) {
      if (!atom->omega_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_omegaz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"angmomx") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_angmomx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"angmomy") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_angmomy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"angmomz") == 0) {
      if (!atom->angmom_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_angmomz;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"tqx") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_tqx;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"tqy") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_tqy;
      vtype[i] = DOUBLE;
    } else if (strcmp(arg[iarg],"tqz") == 0) {
      if (!atom->torque_flag)
        error->all(FLERR,"Dumping an atom property that isn't allocated");
      pack_choice[i] = &DumpCustomUefex::pack_tqz;
      vtype[i] = DOUBLE;
    ...
    // if no trailing [], then arg is set to 0, else arg is int between []
      ...
      pack_choice[i] = &DumpCustomUefex::pack_compute;
      vtype[i] = DOUBLE;
    ...
    // if no trailing [], then arg is set to 0, else arg is between []
      ...
      pack_choice[i] = &DumpCustomUefex::pack_fix;
      vtype[i] = DOUBLE;
    ...
    // variable value = v_name
      ...
      pack_choice[i] = &DumpCustomUefex::pack_variable;
      vtype[i] = DOUBLE;
    ...
    // custom per-atom floating point value = d_ID
      ...
      pack_choice[i] = &DumpCustomUefex::pack_custom;
      vtype[i] = DOUBLE;
...
int DumpCustomUefex::add_compute(char *id)
{
  int icompute;
...
int DumpCustomUefex::add_fix(char *id)
{
  int ifix;
...
int DumpCustomUefex::add_variable(char *id)
{
  int ivariable;
...
int DumpCustomUefex::add_custom(char *id, int flag)
{
  int icustom;
...
int DumpCustomUefex::modify_param(int narg, char **arg)
{
...
void DumpCustomUefex::pack_compute(int n)
{
...
void DumpCustomUefex::pack_fix(int n)
{
...
void DumpCustomUefex::pack_variable(int n)
{
...
void DumpCustomUefex::pack_custom(int n)
{
...
void DumpCustomUefex::pack_id(int n)
{
  tagint *tag = atom->tag;
...
void DumpCustomUefex::pack_molecule(int n)
{
  tagint *molecule = atom->molecule;
...
void DumpCustomUefex::pack_proc(int n)
{
  for (int i = 0; i < nchoose; i++) {
    buf[n] = me;
    n += size_one;
...
void DumpCustomUefex::pack_procp1(int n)
{
  for (int i = 0; i < nchoose; i++) {
    buf[n] = me+1;
    n += size_one;
...
void DumpCustomUefex::pack_type(int n)
{
  int *type = atom->type;
...
void DumpCustomUefex::pack_mass(int n)
{
  int *type = atom->type;
  double *mass = atom->mass;
...
// 2017/11/08 Murashima
void DumpCustomUefex::pack_x(int n)
{
  double rot[3][3];
...
// 2017/11/08 Murashima
void DumpCustomUefex::pack_y(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  ((FixNVEUefex*) modify->fix[ifix_uefex])->inv_rotate_x(rot);
...
// 2017/11/08 Murashima
void DumpCustomUefex::pack_z(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  ((FixNVEUefex*) modify->fix[ifix_uefex])->inv_rotate_x(rot);
...
void DumpCustomUefex::pack_xs(int n)
{
  double **x = atom->x;
...
void DumpCustomUefex::pack_ys(int n)
{
  double **x = atom->x;
...
void DumpCustomUefex::pack_zs(int n)
{
  double **x = atom->x;
...
void DumpCustomUefex::pack_xs_triclinic(int n)
{
  int j;
  double **x = atom->x;
...
void DumpCustomUefex::pack_ys_triclinic(int n)
{
  int j;
  double **x = atom->x;
...
void DumpCustomUefex::pack_zs_triclinic(int n)
{
  double **x = atom->x;
...
void DumpCustomUefex::pack_xu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_yu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_zu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_xu_triclinic(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  ...
  for (int i = 0; i < nchoose; i++) {
    ...
    p[0]=x[j][0] + h[0]*xbox + h[5]*ybox + h[4]*zbox;
    p[1]=x[j][1] + h[1]*ybox + h[3]*zbox;
    p[2]=x[j][2] + h[2]*zbox;
    ((FixNVEUefex*) modify->fix[ifix_uefex])->inv_rotate_pos(rot,p);
    buf[n] = p[0];
    n += size_one;
  }
}
...
void DumpCustomUefex::pack_yu_triclinic(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  int j;
...
void DumpCustomUefex::pack_zu_triclinic(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  int j;
...
void DumpCustomUefex::pack_xsu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_ysu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_zsu(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_xsu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_ysu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_zsu_triclinic(int n)
{
  int j;
  double **x = atom->x;
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_ix(int n)
{
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_iy(int n)
{
  imageint *image = atom->image;
...
void DumpCustomUefex::pack_iz(int n)
{
  imageint *image = atom->image;
...
// 2017/11/15 Murashima
void DumpCustomUefex::pack_vx(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  ((FixNVEUefex*) modify->fix[ifix_uefex])->inv_rotate_v(rot);
  ...
  ((FixNVEUefex*) modify->fix[ifix_uefex])->rotate_v(rot);
}

/* ---------------------------------------------------------------------- */

// 2017/11/15 Murashima
void DumpCustomUefex::pack_vy(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  ((FixNVEUefex*) modify->fix[ifix_uefex])->inv_rotate_v(rot);
...
// 2017/11/15 Murashima
void DumpCustomUefex::pack_vz(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  ((FixNVEUefex*) modify->fix[ifix_uefex])->inv_rotate_v(rot);
...
// 2017/11/15 Murashima
void DumpCustomUefex::pack_fx(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  ((FixNVEUefex*) modify->fix[ifix_uefex])->inv_rotate_f(rot);
  ...
  ((FixNVEUefex*) modify->fix[ifix_uefex])->rotate_f(rot);
}

/* ---------------------------------------------------------------------- */

// 2017/11/15 Murashima
void DumpCustomUefex::pack_fy(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  ((FixNVEUefex*) modify->fix[ifix_uefex])->inv_rotate_f(rot);
...
// 2017/11/15 Murashima
void DumpCustomUefex::pack_fz(int n)
{
  double rot[3][3];
  ((FixNVEUefex*) modify->fix[ifix_uefex])->get_rot(rot);
  ((FixNVEUefex*) modify->fix[ifix_uefex])->inv_rotate_f(rot);
...
void DumpCustomUefex::pack_q(int n)
{
  double *q = atom->q;
...
void DumpCustomUefex::pack_mux(int n)
{
  double **mu = atom->mu;
...
void DumpCustomUefex::pack_muy(int n)
{
  double **mu = atom->mu;
...
void DumpCustomUefex::pack_muz(int n)
{
  double **mu = atom->mu;
...
void DumpCustomUefex::pack_mu(int n)
{
  double **mu = atom->mu;
...
void DumpCustomUefex::pack_radius(int n)
{
  double *radius = atom->radius;
...
void DumpCustomUefex::pack_diameter(int n)
{
  double *radius = atom->radius;
...
void DumpCustomUefex::pack_omegax(int n)
{
...
void DumpCustomUefex::pack_omegay(int n)
{
  double **omega = atom->omega;
...
void DumpCustomUefex::pack_omegaz(int n)
{
  double **omega = atom->omega;
...
void DumpCustomUefex::pack_angmomx(int n)
{
...
void DumpCustomUefex::pack_angmomy(int n)
{
  double **angmom = atom->angmom;
...
void DumpCustomUefex::pack_angmomz(int n)
{
  double **angmom = atom->angmom;
...
void DumpCustomUefex::pack_tqx(int n)
{
...
void DumpCustomUefex::pack_tqy(int n)
{
  double **torque = atom->torque;
...
void DumpCustomUefex::pack_tqz(int n)
{
  double **torque = atom->torque;
