...
/*
  This code is based on dump_custom.h
  Modified by T. Murashima (Tohoku Univ, JPN)
 */

#ifdef DUMP_CLASS

DumpStyle(custom/uefex,DumpCustomUefex)

#else

#ifndef LMP_DUMP_CUSTOM_UEFEX_H
#define LMP_DUMP_CUSTOM_UEFEX_H

#include "dump.h"

namespace LAMMPS_NS {

class DumpCustomUefex : public Dump {
 public:
  DumpCustomUefex(class LAMMPS *, int, char **);
  virtual ~DumpCustomUefex();

 protected:
  int ifix_uefex; // 2017/11/08 Murashima
  ...
  typedef void (DumpCustomUefex::*FnPtrHeader)(bigint);
  ...
  typedef void (DumpCustomUefex::*FnPtrPack)(int);
