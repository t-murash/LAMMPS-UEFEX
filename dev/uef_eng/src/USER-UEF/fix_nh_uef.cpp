  ...
  FixNH(lmp, narg, arg), uefbox(nullptr)
{
  ...
  while (iarg <narg) {
    if (strcmp(arg[iarg],"erate")==0) {
      ...
      if (iarg+2 > narg) error->all(FLERR,"Illegal fix nvt/npt/uef command(test)");
...
void FixNHUef::nve_v()
{
  ...
  //2021/09/21 by TM
  double ex; ex= erate[0]*dtf/2;
  double ey; ey= erate[1]*dtf/2;
  double ez; ez= -ex-ey;
  double e0; e0= exp(-ex);
  double e1; e1= exp(-ey);
  double e2; e2= exp(-ez);
...
int FixNHUef::size_restart_global()
{
  ...
  return FixNH::size_restart_global() +3; // +1 for eng_strain 2021/09/21 by TM
}
...
int FixNHUef::pack_restart_data(double *list)
{
  ...
  list[n++] = strain[0];
  list[n++] = strain[1];
  list[n++] = eng_strain;// 2021/09/21 by TM
  return n;
}
...
void FixNHUef::restart(char *buf)
{
  ...
  */
  ...
  strain[0] = list[n-3];
  strain[1] = list[n-2];
  eng_strain = list[n-1]; // 2021/09/21 by TM
  if(eng_flag){
    double eng_rate_z=eng_rate/(1.0+eng_strain);
    erate[0]=-0.5*eng_rate_z;
    erate[1]=-0.5*eng_rate_z;
